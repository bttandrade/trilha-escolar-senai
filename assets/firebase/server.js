...
aplicacaoExpress.get("/materias", async (req, res) => {
  try {
    ...
    const materias = ["Matemática e suas tecnologias"];
    res.json(materias);
  } catch (erro) {
  ...
  try {
    ...
    if (!materiasValidas.includes(materia)) {
      return res
        .status(400)
        .json({
          erro: `Matéria inválida. Use uma das seguintes: ${materiasValidas.join(
            ", "
          )}.`,
        });
    }
...
aplicacaoExpress.get("/:materia/:assunto/questoes", async (req, res) => {
  try {
    ...
    const referenciaQuestoes = db
      .collection(materiaDecodificada)
      .doc(assunto)
      .collection("Questões");
    ...
    if (snapshotQuestoes.empty) {
      console.log(
        `Nenhuma questão encontrada para ${materiaDecodificada} - ${assunto}`
      );
      return res.json([]);
    }
...
aplicacaoExpress.post("/:materia/:assunto/questoes", async (req, res) => {
  try {
    ...
    const resultados = await Promise.all(
      questoes.map((questao) => referenciaQuestoes.add(questao))
    );
